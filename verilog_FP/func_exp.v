...
reg [31:0] nA;
reg [8:0] delay0, delay1, delay2, delay3, delay4, delay5, delay6;
...
assign A_is_infinite_del = delay6[8];
assign invalid_del = delay6[7];
assign wrenq = delay6[6];
assign wraddrsq = delay6[5:0];             
assign thread_del = delay6[5:4];
...
always @(*) begin
    if (~invalid_del)
        casex (raw_excptns) 
            4'b0001 : backend_exception = _div_by_0_;
            4'b001x : backend_exception = _overflow_;
            4'b01xx : backend_exception = _underflow_;
...
always @(posedge CLK or posedge RESET) begin
    if (RESET) semaphor <= 63'hFFFF_FFFF_FFFF_FFFF;
    else begin
